package NUDT.module.algorithm.cluster;
...
import NUDT.utils.geom.ConvexObject;
...
public class FireCluster extends Cluster{
	...
	// constructor
	public FireCluster(StandardWorldModel world) {
		super(world);
	...
	public void updateFireCondition() {
		...
		double mapArea = this.world.getBounds().getWidth() * this.world.getBounds().getHeight() / 1000000d;
...
//	}
	...
	public void updateConvexHull() {
		...
		for (StandardEntity entity : this.caredEntities) {
			if (entity instanceof Building) {
				building = (Building)entity;
				if (!isBorder && WorldTools.findBorderBuilding(0.9, this.world).contains(entity)) {
					this.setBorder(true);
				}
	...
	public boolean isExpandableToMapCenter() {
		if (isBorder) {
			...
			for (StandardEntity entity : WorldTools.findBorderBuilding(0.9, this.world)) {
				Building building = (Building) entity;

...
import NUDT.utils.geom.ConvexHull_Interface;
...
public abstract class Cluster {
	...
	// constructor
	protected Cluster(StandardWorldModel world) {
...
/* ------------------------------------------- getter and setter ------------------------------------------*/
	...
	public void setDyingClusterAllEntities(Set<EntityID> allEntities) {
		...
		for (EntityID next : allEntities) {
			...
			for (EntityID next_entityID : building.getNeighbours()) {
				StandardEntity this_entity = this.world.getEntity(next_entityID);
				if(this_entity instanceof Building)
				{
				
					this.allEntities.add(next_entityID);
				}
			}
		}
	}
